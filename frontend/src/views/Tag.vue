<template>
  <div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl mx-auto">
      <!-- 标签信息 -->
      <div v-if="tag" class="text-center mb-12">
        <div class="mb-6">
          <span 
      ...
      <div v-else class="text-center py-16">
        ...
        <p class="text-gray-500 dark:text-gray-400 mb-6">该标签下还没有文章</p>
...
export default {
  ...
  setup() {
    ...
    const loadTag = async (id) => {
      try {
        const response = await blogApi.getTagDetail(id)
    ...
    const loadArticles = async (id, page = 1, append = false) => {
      try {
        const response = await blogApi.getTagArticles(id, {
          page,
    ...
    const loadMore = async () => {
      ...
      const tagId = parseInt(route.params.id)
      const nextPage = pagination.value.page + 1
      await loadArticles(tagId, nextPage, true)
    }
